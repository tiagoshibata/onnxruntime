...
// void PRINT_DEBUG_FLOAT(const char* title, const void *p_data, int size) {
//   printf("### DEBUG output - %s:\n", title);
//   for (int i = 0 ; i < size; i++) {
//     printf("  %.4f", ((const float*)p_data)[i]);
//   }
//   printf("\n\n");
// }
...
Status Attention<T>::Compute(OpKernelContext* context) const {
  ...
  //PRINT_DEBUG_FLOAT("STEP1", gemm_data, batch_size * sequence_length * 3 * hidden_size);
  ...
  //PRINT_DEBUG_FLOAT("STEP2", gemm_data_transposed, batch_size * sequence_length * 3 * hidden_size);
  ...
  //PRINT_DEBUG_FLOAT("STEP3", scratch_data, batch_size * num_heads_ * sequence_length * sequence_length);
